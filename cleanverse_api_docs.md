Cleanverse API v3 - Integration Documentation

Cleanverse API v5.6 - Integration Documentation



# [logo](https://www.cleanverse.com)
...
[Generate A-Pass](#generate-apass)
...
[Launch A-Token](#launch-atoken)
[Register A-Token](#register-atoken)
[Launch Wrapped A-Token](#launch-wrapped-atoken)
[Register Wrapped A-Token](#register-wrapped-atoken)
[Query Apply Status](#query-apply-status)
[List My A-Tokens](#list-my-atokens)
[A-Token Apply Webhook](#atoken-apply-webhook)
[Add A-Token Rule](#add-atoken-rule)
[Query A-Token Rule](#query-atoken-rules)
[Remove A-Token Rule](#remove-atoken-rule)
[Verify A-Token Paused](#verify-atoken-paused)
[Set A-Token Paused](#set-atoken-paused)
[Add Institutional Deposit Whitelist](#add-whitelist-institutional)
[Remove Institutional Deposit Whitelist](#remove-whitelist-institutional)
[Restore Institutional Deposit Whitelist](#restore-whitelist-institutional)

Validator Compliance

▼

[Module Overview](#module-validator-compliance)
[Grant Registrar Role](#validator-grant)
[Register Compliance Pool](#validator-register)
[Query Registration Status](#validator-is-register)
[Set Pool Rules](#validator-set-rule)
[Add Pool Rule](#validator-add-rule)
[Remove Pool Rule](#validator-remove-rule)
[Query Pool Rules](#validator-rules)
[Verify User Compliance](#validator-verify)
[Set Pool Pause State](#validator-set-paused)
[Query Pool Pause State](#validator-is-paused)

Fiat Ramp

▼

[Module Overview](#module-fiat-ramp)
[Query Supported Countries](#query-ramp-countries)
[Query Fiat Currencies](#query-ramp-fiat-currencies)
[Query Crypto Currencies](#query-ramp-crypto-currencies)
[Query Payment Methods](#query-ramp-payment-methods)
[Request Quote](#query-ramp-quote)
[Create Widget URL](#create-ramp-widget-url)
[Query Order](#query-ramp-order)

Common Queries

▼

[Query Supported A-Token List](#query-deposit-atoken-list)
[Query A-Pass List](#query-apass-list)
[Query A-Pass](#query-apass)
[Verify A-Pass](#verify-apass)
[Query Deposit Address](#query-deposit-address)
[Query Institution Whitelist Address](#query-institution-whitelist)
[Query Transactions](#query-txs)
[Query Institution Transactions](#institution-query-txs)
[Download Travel Rule Report](#download-travel-rule)
[Institution Faucet](#faucet)

Appendix

[ISO Country Or Region Codes](#country-region-codes)
[Currency Codes](#currency-codes)

© 2026 Cleanverse International Pte Ltd. All rights reserved.

For support, contact: [support@cleanverse.com](mailto:contact@cleanverse.com)
...
| --- | --- | --- |
| **v5.6** | 2026-07-21 | **Validator pool compliance rules — country allow/deny**: Pool [Rule object](#validator-rule-object) adds optional `is_black_list` and `countries` (same semantics as A-Token rules). Applies to [`POST /validator/register`](#validator-register), [`POST /validator/set_rule`](#validator-set-rule), [`POST /validator/add_rule`](#validator-add-rule), and [`POST /validator/rules`](#validator-rules) responses. Fields are optional for backward compatibility. Prefer waiting for the previous write transaction to confirm before issuing another rule mutation. |
| **v5.5** | 2026-07-13 | **A-Pass country tags & A-Token country-based compliance rules**: [`POST /generate_apass`](#generate-apass) derives A-Pass country tags from `identityDataList[].issuingCountryISO2` (ISO 3166-1 alpha-2); [`POST /query_apass`](#query-apass) and [`POST /query_apass_list`](#query-apass-list) return those tags on registered records. A-Token **compliance rules** add optional `is_black_list` and `countries` so institutions can allow or deny by country, in addition to existing tier/group constraints ([`POST /atoken/launch`](#launch-atoken), [`POST /atoken/launch_wrapped_atoken`](#launch-wrapped-atoken), [`POST /atoken/add_rule`](#add-atoken-rule), [`POST /atoken/rules`](#query-atoken-rules)). |
...
All API requests require the `api-id` header. Selected endpoints additionally require AES-encrypted request bodies (see [Encryption](#encryption)).

Base path for all endpoints in this document: `{environment_url}/api/cooperate` (see [Environment](#environment)).
...
**Plain JSON (no encryption):** All [Fiat Ramp](#module-fiat-ramp) endpoints (`/query_ramp_*`, `/create_ramp_widget_url`) and Validator read endpoints (`is_register`, `rules`, `verify`, `is_paused`) accept unencrypted JSON bodies.
...
1. Convert your JSON request body to a string
2. Decode Base64-encoded api-key,then use the api-key as the AES encryption key
3. Encrypt the JSON string using AES algorithm with the api-key
4. Encode the encrypted bytes to Base64-encoded string
5. Send the Base64-encoded encrypted string in the data field.

### Decryption Process

1. Receive the Base64-encoded encrypted response
2. Decode the Base64 string to bytes
3. Decrypt the bytes using AES with your api-key
...
| 403 | Forbidden | - | 1.Invalid or missing api-id.  2.Unallowed IP.  3.Unauthorized access.  4.Data decryption failure. |
...
* Poll [Query Apply Status](#query-apply-status), wait for an [Apply Result Webhook](#atoken-apply-webhook) (if you supplied `callback_url`), or check the **Member** platform until you see a final outcome. Issuance is successful when the status is `ISSUED`.
...
This endpoint requires request body encryption. See [Encryption](#encryption) for the algorithm, IV, key derivation, and steps. The HTTP request body must be a JSON object with a single field `data` whose value is the Base64-encoded ciphertext of the plaintext JSON below.
...
| `callback_url` | string | No | Optional HTTPS/HTTP webhook URL for apply result notification when `applyStatus` reaches a terminal state. Max 512 characters. See [A-Token Apply Result Webhook](#atoken-apply-webhook). |
...
| Field | Type | Description |
| --- | --- | --- |
| `requestId` | string | Application request identifier. Use this value to query the final issuance result via [Query Apply Status](#query-apply-status). |
...
* The response only confirms submission and includes a `requestId`.
* Poll [Query Apply Status](#query-apply-status) or check the **Member** platform until you see a final outcome. The application succeeds when the status is `ISSUED`.
* The same application can be started from the **Member** platform instead of this API.
...
| `callback_url` | string | No | Optional webhook URL for apply result notification. See [A-Token Apply Result Webhook](#atoken-apply-webhook). |
...
| Field | Type | Description |
| --- | --- | --- |
| `requestId` | string | Application request identifier. Use this value to query the final result via [Query Apply Status](#query-apply-status). |
...
* The response only confirms submission and includes a `requestId`.
* Poll [Query Apply Status](#query-apply-status) or check the **Member** platform until you see a final outcome. Issuance is successful when the status is `ISSUED`.
* The same application can be started from the **Member** platform instead of this API.

#### After `ISSUED` (on-chain)

1. Using `admin_address`, grant `MINTER_ROLE` on the Wrapped A-Token to the **access\_core** contract. Look up the **access\_core** address with [Query Supported A-Token List](#query-deposit-atoken-list).
2. When a user sends the native (origin) token from a **whitelisted institution address** to their **deposit address**, **access\_core** locks the native balance and mints A-Tokens to the user’s wallet at a **1:1** ratio.
...
Query the processing status of an A-Token application by `requestId`. Use this endpoint to obtain the final result after submitting an application via: [Launch A-Token](#launch-atoken), [Register A-Token](#register-atoken), [Launch Wrapped A-Token](#launch-wrapped-atoken), or [Register Wrapped A-Token](#register-wrapped-atoken). An issuance is considered successful only when `applyStatus` becomes `ISSUED`.
...
Returns a paginated list of **this institution's** A-Token apply rows from both direct launch/register and wrapped launch/register flows. Results are sorted by submission time (`createTime`) descending. For a single application’s full status, rejection reason, issue error, and webhook delivery fields, use [Query Apply Status](#query-apply-status).
...
| `apply_status` | string | No | Filter by apply status. Values: `PENDING`, `APPROVED`, `ISSUING`, `ISSUED`, `REJECTED`, `ISSUE_FAILED`. Treat `ISSUED` as success; `REJECTED` and `ISSUE_FAILED` are terminal failures. See [Query Apply Status](#query-apply-status) for transitions and detail fields. |
...
| `applyStatus` | `PENDING`, `APPROVED`, `ISSUING`, `ISSUED`, `REJECTED`, or `ISSUE_FAILED`. Poll [Query Apply Status](#query-apply-status) by `requestId` for `rejectReason`, `issueErrorMsg`, and callback fields. |
...
You can poll [Query Apply Status](#query-apply-status) for the same outcome and for delivery status fields (`callbackStatus`, `callbackAttempts`, `callbackLastError`).
...
1. Read the raw HTTP body bytes as UTF-8 JSON string (exact bytes Cleanverse POSTed — do not re-serialize with different spacing).
2. Compute `HMAC-SHA256(body, Base64.decode(api_key))`.
3. Compare lowercase hex digest to `X-Cleanverse-Signature`.
...
`timestamp` is Unix time in **seconds**. In webhook payloads, `data.issuedAt` uses ISO-8601 with timezone; [Query Apply Status](#query-apply-status) returns `issuedAt` as `yyyy-MM-dd HH:mm:ss` (server local time).
...
Each call adds new whitelist rows for the entity. To deactivate or reactivate an existing address, use [Remove](#remove-whitelist-institutional) or [Restore](#restore-whitelist-institutional) instead of calling add again.
...
See [Remove](#remove-whitelist-institutional) / [Restore](#restore-whitelist-institutional) for lifecycle changes on existing rows.

## Remove Institutional Deposit Whitelist

Issue Member

Deactivates one or more **whitelisted sender addresses** for **native-token** deposits into your institution’s configured deposit address. Applies only to **Wrapped A-Tokens your institution has issued** (same scope as [Add Institutional Deposit Whitelist](#add-whitelist-institutional)).
...
Use [Restore Institutional Deposit Whitelist](#restore-whitelist-institutional) to reactivate a previously removed address.
...
This endpoint requires request body encryption. See [Encryption](#encryption). Send `{"data":"<Base64 ciphertext>"}`.
...
Reactivates one or more previously removed **whitelisted sender addresses** for institutional native-token deposits. Same scope as [Add](#add-whitelist-institutional) and [Remove](#remove-whitelist-institutional) institutional deposit whitelist.
...
Same as [Remove Institutional Deposit Whitelist](#remove-whitelist-institutional): `chain`, `symbol`, optional `assetAddress` / `asset_address`, and `walletAddresses` / `wallet_addresses`.
...
All Validator endpoints use the path prefix `/validator/` under the cooperate base URL (see [Environment](#environment)).
...
Encryption uses the same AES/CBC scheme as other cooperate APIs (see [Encryption](#encryption)).
...
Encrypt the plaintext JSON below per [Encryption](#encryption). Send `{"data":"<Base64 ciphertext>"}`.
...
| `rule` | object | Yes | Initial compliance rule (see [Rule object](#validator-rule-object)) |
...
| `rule` | object | Yes | Rule to apply ([Rule object](#validator-rule-object)) |
...
Appends a compliance rule to a registered pool (optional `is_black_list` / `countries`; see [Rule object](#validator-rule-object)). Encrypted request body. Ensure the previous write (e.g. `set_rule`) has confirmed on-chain first.
...
| `chain` | string | Yes | Blockchain network |
| `contract_address` | string | Yes | Registered pool address |
| `rule` | object | Yes | Rule to append ([Rule object](#validator-rule-object)) |
...
Returns all compliance rules configured for a registered pool (each item is a [Rule object](#validator-rule-object), including optional `is_black_list` / `countries`). Plain JSON request.
...
1. **Discover markets (optional)** — Call metadata endpoints to list supported countries, fiat and crypto currencies, and payment methods.
2. **Request Quote** — Submit trade parameters (side, currencies, network, amount). Cleanverse returns a priced quote and a server-issued `quoteToken`.
3. **Create Widget URL** — Submit only `quoteToken`, the end-user wallet, and optional contact / risk fields. Cleanverse applies the quoted amounts and fees; do not send price fields on this step.
4. **Present the widget** — Redirect or embed the returned `widgetUrl` so the user completes payment with the ramp provider.
5. **Query Order** — Poll order status and retrieve the quote snapshot recorded at creation time.

#### Quote token (`quoteToken`)

* Issued by Cleanverse when you call [Request Quote](#query-ramp-quote).
* **Single-use:** consumed when [Create Widget URL](#create-ramp-widget-url) succeeds. A second attempt with the same token returns `[RM_007]`.
...
* The end-user wallet must have a registered, non-frozen **A-Pass** on the same `chain` you pass to `Create Widget URL` (see [Generate A-Pass](#generate-apass)).
...
On failure, HTTP status remains `200`; check `code` and `message`. Parameter validation failures typically use `code` `0002` with `[400]` in `message`. Business failures use `code` `0002` with bracketed sub-codes such as `[RM_007]` (see [Response Codes](#response-codes)).
...
| `paymentMethod` | string | Yes | Payment method ID from [Query Payment Methods](#query-ramp-payment-methods) |
...
| Field | Type | Description |
| --- | --- | --- |
| `quoteToken` | string | Single-use token for [Create Widget URL](#create-ramp-widget-url) (15-minute validity) |
...
Creates a ramp order and returns a hosted widget URL for the end user. You must call [Request Quote](#query-ramp-quote) immediately beforehand and pass the returned `quoteToken`.
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `quoteToken` | string | Yes | `quoteToken` from [Request Quote](#query-ramp-quote) |
...
| Field | Type | Description |
| --- | --- | --- |
| `orderId` | string | Cleanverse order ID (e.g., `RAMP2072497313324244992`). Use with [Query Order](#query-ramp-order). |
...
| Field | Type | Required | Description |
| --- | --- | --- | --- |
| `orderId` | string | Yes | `orderId` from [Create Widget URL](#create-ramp-widget-url) |
...
| `quote` | object | Quote snapshot at creation (same shape as [Request Quote](#query-ramp-quote) response, including `quoteToken` and `quoteId`) |
...
#### Token Info Object (origin\_token / atoken)
...
#### Token Whitelist Object (token\_whitelist item)
...
#### Tx Group Object (tx\_groups item)
...
For support, contact: [support@cleanverse.com](mailto:api-support@cleanverse.com)
